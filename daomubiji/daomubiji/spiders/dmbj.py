...
class DmbjSpider(RedisSpider):
    ...
    def parse_title(self, response):  # 提取子网页信息
        ...
        for each in chapter_list:
            ...
            yield Request(chapter_link[chapter_link_flag], callback=self.parse_content, meta={'item': item})
            chapter_link_flag += 1
    ...
    def parse(self, response):  # 程序从这个函数开始执行
        ...
        book_link = selector.xpath(
            '//article/p/a/@href').extract()  # 抓取盗墓笔记每本书的链接
        ...
        for each in book_filed:
            ...
            if pos == -1:  # 只抓取符合我们格式规定的书
                continue
            # 调用parse_title函数
            yield Request(book_link[link_flag], callback=self.parse_title)
            link_flag += 1